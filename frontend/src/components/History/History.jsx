...
function History() {
  ...
  const handleQtyChange = (type, value) => {
    ...
    const max = currentReturn ? currentReturn.quantity - (currentReturn.total_return || 0) : 0;
  ...
  const filteredHistory = history.filter((his) => {
    ...
    const matchSearch = his.name?.toLowerCase().includes(search.toLowerCase()) || his.full_name?.toLowerCase().includes(search.toLowerCase()) || his.frist_name?.toLowerCase().includes(search.toLowerCase()) || his.last_name?.toLowerCase().includes(search.toLowerCase());
      ...
      <Container className="py-4" style={{ backgroundColor: "#F5F5F5", minHeight: "100vh", marginTop: "80px" }}>
              ...
              <tbody>
                {filteredHistory
                  ...
                  .map((r) => (
                    ...
                    <tr key={r.request_id}>
                      <td>{r.request_id}</td>
                      {role === 1 && <td onClick={() => handleShow(r.full_name || `${r.frist_name} ${r.last_name}`, r.email, r.phone_number, r.team)}>{r.full_name && r.full_name.trim() !== "" ? r.full_name : `${r.frist_name} ${r.last_name}`}</td>}
                      ...
                      {role === 1 && (
                        <td>
                          ...
                          {r.status_name === "คืนไม่ครบ" && r.quantity !== r.total_return && (
                            <Button
                              variant="warning"
                              className="w-100 my-1"
                              onClick={() => {
        ...
        <Modal show={showReturnModal} onHide={() => setShowReturnModal(false)} centered>
          ...
          <Modal.Body>
            {currentReturn && (
              <>
                <p>
                  จำนวนที่คืน <strong>(รวมไม่เกิน {currentReturn.quantity - (currentReturn.total_return || 0)} ชิ้น)</strong>
                </p>
                <Row className="mb-3">
                  <Col>
                    <Form.Label>สมบูรณ์</Form.Label>
                    <Form.Control type="number" min="0" value={returnGoodQty} onChange={(e) => handleQtyChange("good", e.target.value)} />
                  </Col>
                  <Col>
                    <Form.Label>ชำรุด</Form.Label>
                    <Form.Control type="number" min="0" value={returnDamagedQty} onChange={(e) => handleQtyChange("damaged", e.target.value)} />
                  </Col>
                  <Col>
                    <Form.Label>สูญหาย</Form.Label>
                    <Form.Control type="number" min="0" value={returnLostQty} onChange={(e) => handleQtyChange("lost", e.target.value)} />
                  </Col>
                </Row>

                <Form.Group className="mb-3">
                  <Form.Label>หมายเหตุ (ถ้ามี)</Form.Label>
                  <Form.Control as="textarea" rows={2} value={returnComment} onChange={(e) => setReturnComment(e.target.value)} />
                </Form.Group>

                <Form.Group className="mb-3">
                  <Form.Label>ชื่อผู้รับคืน</Form.Label>
                  <Form.Control type="text" placeholder="กรอกชื่อผู้รับคืน" value={receiverName} onChange={(e) => setReceiverName(e.target.value)} />
                </Form.Group>

                <Form.Group className="mb-3">
                  <Form.Label>ชื่อผู้คืน</Form.Label>
                  <Form.Control type="text" placeholder="กรอกชื่อผู้คืน" value={returnedName} onChange={(e) => setReturnedName(e.target.value)} />
                </Form.Group>
                <p>
        ...
        <Modal show={showCancelModal} onHide={handleCloseCancel}>
          ...
          <Modal.Body>
            ...
            <p>
              <strong>ผู้ยืม:</strong> {selectedRequest?.full_name && selectedRequest.full_name.trim() !== "" ? selectedRequest.full_name : `${selectedRequest?.frist_name || ""} ${selectedRequest?.last_name || ""}`} ({selectedRequest?.team || "-"})
            </p>
            ...
            <p>
              <strong>วันที่ยืม:</strong> <DateComponent dateString={selectedRequest?.request_date} />
            </p>
            <p>
            ...
            <Form.Group className="mt-3">
              ...
              <Form.Control as="textarea" rows={3} placeholder="กรุณาระบุเหตุผล" value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} />
            </Form.Group>

            <p className="mt-3">
              <strong>ชื่อผู้ใช้งาน:</strong> {user?.full_name && user.full_name.trim() !== "" ? user.full_name : `${user?.frist_name || ""} ${user?.last_name || ""}`.trim() || "-"}
            </p>
            <Form.Group className="mt-2">
              <Form.Label>
                <strong>ชื่อผู้ยกเลิก (ต้องกรอกเอง)</strong>
              </Form.Label>
              <Form.Control type="text" placeholder="ระบุชื่อ-นามสกุลของผู้ยกเลิก" value={canceledByInput} onChange={(e) => setCanceledByInput(e.target.value)} />
            </Form.Group>

            <p className="mt-3">
              <strong>วันที่ยกเลิก:</strong> <DateComponent dateString={new Date().toISOString()} />
            </p>
          </Modal.Body>
          ...
          <Modal.Footer>
            ...
            <Button variant="danger" onClick={() => confirmCancel(selectedRequest.request_id)} disabled={!cancelReason.trim() || !canceledByInput.trim()}>
              ยืนยันการยกเลิก
            </Button>
          </Modal.Footer>
        </Modal>
        ...
        <Modal show={showApproveModal} onHide={() => setShowApproveModal(false)}>
          ...
          <Modal.Body>
            <Form.Group>
              ...
              <Form.Control as="textarea" rows={3} value={approveNote} onChange={(e) => setApproveNote(e.target.value)} />
            </Form.Group>
          </Modal.Body>
          ...
          <Modal.Footer>
            ...
            <Button
              ...
              onClick={() => {
                updateStatus(approveTarget.request_id, "อนุมัติแล้ว", approveTarget.quantity, approveTarget.product_id);
